...
class TeacherControllerTest {
	...
	void getTeacherByIdTest() throws Exception {
		...
		TeacherDto teachDto = teacherMapper.toDto(teacher);
		String jsonResponse = objectMapper.writeValueAsString(teachDto);
		mockMvc.perform(get("/api/teacher/"+teacher.getId()).contentType(MediaType.APPLICATION_JSON_VALUE)).andExpect(status().isOk())
	...
	void getAllTeachersTest() throws Exception {
		...
		mockMvc.perform(get("/api/teacher").contentType(MediaType.APPLICATION_JSON_VALUE)).andExpect(status().isOk())
		.andExpect(content().json(jsonResponse, true));
	}
}
